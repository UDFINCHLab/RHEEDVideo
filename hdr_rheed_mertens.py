...
EXPOSURE_CYCLE = [3000.0, 9000.0, 20000.0]  # microseconds
...
def main():
    ...
    def capture_loop():
        ...
        while not stop_event.is_set():
            # Lock gain
            if hasattr(cam, "set_gain_db") and last_gain != GAIN_CONSTANT:
                cam.set_gain_db(GAIN_CONSTANT)
                last_gain = GAIN_CONSTANT

            # HDR exposure logic continues
            ...
            # ---- Robust HDR display normalization ----
            # prevents white saturation while preserving detail
            p1, p99 = np.percentile(fused_gray, (1, 99))

            fused_gray = np.clip((fused_gray - p1) / (p99 - p1 + 1e-6), 0, 1)

            fused_8 = (fused_gray * 255).astype(np.uint8)
