...
EPS = 1e-6
USE_HALF_RESOLUTION_FOR_HDR = False   # set True only if speed is too slow
...
def main():
    ...
    if hasattr(cam, "set_gamma_enabled"):
        gamma_enabled = cam.set_gamma_enabled(False)
    ...
    def capture_loop():
        ...
        while not stop_event.is_set():
            ...
            # --- discard one unstable frame (sensor latency) ---
            cam.get_frame()
            ...
            # Optional downscale only if needed for speed
            if USE_HALF_RESOLUTION_FOR_HDR:
            ...
            # ---------------- TRUE HDR RECONSTRUCTION ----------------
            # OpenCV expects exposure times in seconds
            times = np.array(cycle, dtype=np.float32).reshape(-1, 1) / 1e6

            # Debevec works more reliably with 3-channel uint8 images
            ldr_imgs = []
            for img in hdr_buffer:
                if img.ndim == 2:
                    ...
                    img_bgr = img.copy()
                ldr_imgs.append(img_bgr)

            # Debevec radiance reconstruction
            try:
                radiance = merge_debevec.process(ldr_imgs, times=times)
            except Exception as e:
                print("❌ Debevec HDR failed:", e)
                hdr_buffer.clear()
                continue

            # radiance is float32 BGR
            radiance = np.maximum(radiance.astype(np.float32), EPS)

            # Convert to grayscale radiance for your downstream ROI pipeline
            radiance_gray = cv2.cvtColor(radiance, cv2.COLOR_BGR2GRAY)

            # ---------------- LOG TONE MAPPING ----------------
            # ---------------- IMPROVED LOG TONE MAPPING ----------------
            log_hdr = np.log1p(radiance_gray * 3.0)

            p_low, p_high = np.percentile(log_hdr, (1, 99))

            log_hdr = np.clip((log_hdr - p_low) / (p_high - p_low + EPS), 0, 1)

            fused_8 = (log_hdr * 255).astype(np.uint8)
            
            # Local contrast enhancement (very good for RHEED)
            clahe = cv2.createCLAHE(clipLimit=2.5, tileGridSize=(8,8))
            fused_8 = clahe.apply(fused_8)

            # ---------------- TEMPORAL SMOOTHING ----------------
            if not hasattr(capture_loop, "prev_hdr"):
                capture_loop.prev_hdr = fused_8.copy()

            alpha = 0.45   # previous-frame weight
            fused_8 = cv2.addWeighted(
                capture_loop.prev_hdr, alpha,
                fused_8, 1.0 - alpha,
                0
            )
            ...
            if ml_capture and ml_writer is not None:
                gray_local = output_frame if output_frame.ndim == 2 else cv2.cvtColor(output_frame, cv2.COLOR_BGR2GRAY)
        ...
        while True:
            ...
            if HDR_MODE:
                ...
                if key == ord('7'):
                    ...
                    GAIN_CONSTANT = min(gain_max, GAIN_CONSTANT + gain_step_hdr)
                    if hasattr(cam, "set_gain_db"):
                        cam.set_gain_db(GAIN_CONSTANT)
                    print("HDR Gain:", GAIN_CONSTANT)

                elif key == ord('f'):
                    GAIN_CONSTANT = max(gain_min, GAIN_CONSTANT - gain_step_hdr)
                    if hasattr(cam, "set_gain_db"):
                        cam.set_gain_db(GAIN_CONSTANT)
                    print("HDR Gain:", GAIN_CONSTANT)    
            ...
            if ml_toggle_request:
                ...
                if not ml_capture:
                    if last_gray_shape is None:
                        ...
                        if hasattr(cam, "cam") and hasattr(cam.cam, "AcquisitionResultingFrameRate"):
                            hardware_fps = cam.cam.AcquisitionResultingFrameRate.GetValue()
                        else:
                            hardware_fps = cam.get_fps()
